...
export const BugHighlighter: React.FC<BugHighlighterProps> = ({
  id,
  ids,
  children,
  className,
  ...
  const [savedDetailsMap, setSavedDetailsMap] = useState<Record<string, BugDetails>>({});
  ...
  const handleClick = () => {
    if (isActive) {
      ...
      const snapshot: Record<string, BugDetails> = {};
  ...
  const renderBugContent = (details: BugDetails | null | undefined) => {
    if (!details) return null;
    return (
      <div className="mt-4 space-y-6 animate-in fade-in duration-150">
        {!showCompare ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <Badge
                    variant="destructive"
                    className="bg-red-500/15 text-red-600 hover:bg-red-500/25 border-0"
                  >
  ...
  return (
    <>
      <div
        ...
        onClick={handleClick}
      >
        <div className={isActive ? 'highlight-pulse rounded-lg' : ''}>{children}</div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-6xl w-[95vw] max-h-[90vh] overflow-y-auto duration-150 data-[state=open]:zoom-in-100 data-[state=closed]:zoom-out-100">
          <DialogHeader>
            <div className="flex items-center gap-2">
              ...
              <DialogTitle className="text-xl">
                {validIds.length > 1
                  ...
                  : singleDetails?.name || bugName || 'Potential Bug Detected'}
              </DialogTitle>
            </div>
            <DialogDescription>
              {validIds.length > 1
          ...
          {validIds.length > 1 ? (
            <Tabs defaultValue={validIds[0]} className="w-full">
              <TabsList className="flex flex-row w-full h-auto justify-start items-center overflow-x-auto gap-2 bg-muted p-1 flex-wrap">
                {validIds.map((tid) => (
                  <TabsTrigger
                    key={tid}
                    value={tid}
                    className="py-2 px-3 text-xs md:text-sm font-medium whitespace-nowrap data-[state=active]:bg-background data-[state=active]:text-foreground rounded-sm"
                  >
