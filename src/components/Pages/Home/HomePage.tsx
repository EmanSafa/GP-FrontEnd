...
const HomePage = () => {
  ...
  const { data: productReviews } = useGetProductReviews(Number(featuredProductId), {
    ...
    <div className="animate-in fade-in zoom-in duration-500">
          ...
          {[
            ...
            <CarouselItem key={index}>
              <div className="w-full">
                <Card
                  className={`${slide.bg} transform transition-transform duration-500 hover:scale-[1.01]`}
                >
                  <CardContent
                  ...
                  >
                    ...
                    >
                      ...
                      <Link to="/shop">
                        <Button
                          variant={'default'}
                          className="mt-5 w-full md:w-40 text-lg rounded-sm cursor-pointer   hover:bg-plate-7 transition-all hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl"
                        >
                          Shop Now
                        </Button>
                      </Link>
      ...
      <div>
        <div className="w-[88%] mx-auto mt-7">
          <h1 className="lg:text-2xl sm:text-sm md:text-lg font-semibold">Shop by Category</h1>
            ...
            <CarouselContent className=" gap-3">
              {categories?.map((cat) => (
                <CarouselItem key={cat.id} className="basis-auto group">
                  <div
                    onClick={() => {
                      void navigate({ to: '/shop', search: { categoryId: cat.id } });
                    }}
        ...
        <div className="w-[88%] mx-auto mt-10 ">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-500">
            ...
            {productReviews?.reviews?.slice(0, 3).map((review, index) => (
              <UserFeedback
                key={review.id}
                img={
                  productsData?.products?.[0]?.main_image_url ||
